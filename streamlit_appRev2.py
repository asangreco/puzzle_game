...
# track last processed upload (prevents autosolve-on-click regression)
...
# Upload (guarded so it runs only when file changes)
    ...
    if st.button("Solve (A* - optimal)"):
        if not is_solvable(st.session_state.state):
            ...
            if path is None:
                ...
                # Apply the solution immediately and refresh (skip broken playback)
                st.session_state.state = path[-1]          # GOAL
                st.session_state.move_count += len(path) - 1
                st.session_state.solution = None           # avoid entering playback block
                st.session_state.sol_index = 0
                st.success(f"Solved in {len(path)-1} moves (time {t1-t0:.2f}s).")
                st.rerun()
        ...
        with cols[c]:
            if st.button(f"{tile_num}", key=f"tile_{idx}"):
                ...
                if abs(zr - tr) + abs(zc - tc) == 1:
                    ...
                    st.rerun()  # <-- fix ghost image by refreshing view

            st.image(tiles[tile_num], use_container_width=True)

# (Playback section intentionally unchanged; it won't be reached because solution=None after Solve.)
...
- Press *Solve (A\**)* to compute and apply the optimal solution immediately.
""")
